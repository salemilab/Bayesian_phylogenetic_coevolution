...
option_list <- list( 
  ...
  make_option(c("-d", "--days"), action="store",type="integer", default=NULL, 
              help="Most recent time point in days",
              metavar="number"),
  ...
  make_option(c("-m", "--mode"), action="store",type="integer", default=1, 
              help="In the event that a branch covers more than one time window, choose if analysis will be done by assigning these branches to their minimum or maximum window. Choose 1 for minimum window or 2 for maximum window.")
)
  ...
  for (j in 1) {
    if (is_empty(deletions.min) == TRUE){
      ...
      df.min = merge(slid.win.min, deletions.min, by = c('label','muts','site','window','min_win','node','parent',
                                                         'origin_dpi','end_dpi', 'origin_tissue','end_tissue'), all = T)
      
      write.csv(df.min, paste0(parser$output,"_branch-site_mutations_min.win.csv"), quote=F, row.names=F)
    }
  }
  ...
  for (pair in 1) {
    if(is_empty(min.result_pairs)==TRUE){
      ...
      strength_distribution.min = ggplot(final.min, aes(x=strength)) +
        geom_density() + xlab("Strength (posterior)") + ylab("Number of pairs of sites") +
        theme_classic()
...
#this will put the sequence only in the last window they appear
  ...
  #Now remove indels from the larger mutation datatable and merge it with the deletions table
      ...
      df.max = merge(slid.win.max, deletions.max, by = c('label','muts','site','window','max_win','node','parent',
                              'origin_dpi','end_dpi', 'origin_tissue','end_tissue'), all = T)
  write.csv(df.max, paste0(parser$output,"_branch-site_mutations_max.win.csv"), quote=F, row.names=F)
    }
  }
